...
function log(message, ...args) {
  import_node_process2.default.stderr.write(`${import_node_util.default.format(message, ...args)}${import_node_os.EOL}`);
}
...
var invokeAsync = (callback, eventName, logger27, telemetryClient, correlationId) => {
  return (...args) => {
    logger27.trace(`Executing function ${eventName}`);
    ...
    return callback(...args).then((response) => {
      logger27.trace(`Returning result from ${eventName}`);
      inProgressEvent?.end({
        success: true
      });
      return response;
    }).catch((e) => {
      logger27.trace(`Error occurred in ${eventName}`);
      try {
        logger27.trace(JSON.stringify(e));
      } catch (e2) {
        logger27.trace("Unable to print error message.");
      }
      inProgressEvent?.end({
        success: false
      }, e);
...
var AccountEntity = class _AccountEntity {
   ...
   * @param authType
   */
  static generateHomeAccountId(serverClientInfo, authType, logger27, cryptoObj, idTokenClaims) {
    if (!(authType === AuthorityType.Adfs || authType === AuthorityType.Dsts)) {
      ...
      logger27.warning("No client info in response");
    }
...
var PopTokenGenerator = class {
  ...
  async generateCnf(request, logger27) {
    ...
    const reqCnf = await invokeAsync(this.generateKid.bind(this), PerformanceEvents.PopTokenGenerateCnf, logger27, this.performanceClient, request.correlationId)(request);
...
var ResponseHandler = class _ResponseHandler {
  constructor(clientId, cacheStorage, cryptoObj, logger27, serializableCache, persistencePlugin, performanceClient) {
...
function buildAccountToCache(cacheStorage, authority, homeAccountId, base64Decode, correlationId, idTokenClaims, clientInfo, environment, claimsTenantId, authCodePayload, nativeAccountId, logger27) {
  logger27?.verbose("setCachedAccount called");
...
var uuidFunction = typeof globalThis?.crypto?.randomUUID === "function" ? globalThis.crypto.randomUUID.bind(globalThis.crypto) : import_node_crypto.randomUUID;
  ...
  async getAzureCliAccessToken(resource, tenantId, subscription, timeout) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        import_child_process.default.exec(command, { cwd: cliCredentialInternals.getSafeWorkingDir(), timeout }, (error, stdout, stderr) => {
...
// build/business-central-client.js
var BusinessCentralClient = class {
  config;
  companyId;
  credential;
  constructor(config) {
...
// build/index.js
var BC_URL_SERVER = process.env.BC_URL_SERVER;
var BC_COMPANY = process.env.BC_COMPANY;
var BC_AUTH_TYPE = process.env.BC_AUTH_TYPE || "azure_cli";
...
var bcClient = new BusinessCentralClient({
